...
function PixelHeatmap() {
  ...
  const importance = useMemo(() => (tree ? featureImportance(tree, FEATURE_PIXELS) : null), [tree]);
...
export function Results() {
  ...
  useEffect(() => {
    ...
    const entry: LeaderboardEntry = {
      ...
      summary: `max depth ${hyperparams.maxDepth}, ${datasetConfig.samples} samples, ${tree.leafCount} leaves`,
    };
    const updated = addLeaderboardEntry(entry);
    setBoard(updated);
    setCurrentId(id);
  }, [tree, evaluation, hyperparams.maxDepth, datasetConfig.samples]);
      ...
      <main className="mx-auto max-w-5xl px-6 pb-24 pt-2 sm:px-8">
        <div className="mb-8 text-center">
          ...
          <p className="mx-auto mt-3 max-w-xl text-[17px] leading-relaxed text-ink-soft">
            Performance on handwritten digits the tree never saw during training.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[auto_1fr]">
          <div className="flex flex-col gap-6">
            <Card className="p-6">
              ...
              <p className="mb-4 text-sm text-ink-soft">Where on the digit the tree chose to look.</p>
              <PixelHeatmap />
            </Card>
